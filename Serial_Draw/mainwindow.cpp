...
void MainWindow::on_send_data_clicked() {
    if (ui->add_crlf->checkState()) { // 检查是否需要自动添加回车换行
        COM->write(ui->textEdit->toPlainText().toLatin1()); // 写入数据
        COM->write("\r\n"); // 添加回车换行
        ui->textBrowser->insertPlainText("\r\n" + QDateTime::currentDateTime().toString("MM-dd HH:mm:ss:zzz") +
                                         tr(" - send:") + "\t" + tr(ui->textEdit->toPlainText().toLatin1())
                                         + "\\r\\n"); // 发送数据回显
    } else {
        QByteArray data = ui->textEdit->toPlainText().toLatin1(); // 获取文本框中的数据
        COM->write(data); // 发送数据
        ui->textBrowser->insertPlainText("\r\n" + QDateTime::currentDateTime().toString("MM-dd HH:mm:ss:zzz") +
...
void MainWindow::on_timed_send_stateChanged(int state) {
    if (state == Qt::Checked) {
        ...
        QPixmap pixmap(":/image/qt_picture/TablerClockCheck.png"); // 加载启动时的图片
